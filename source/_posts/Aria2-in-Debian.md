...
Debian 系的系统下可以直接命令安装，Rh 系的将安装命令换成 `yum` 即可
...
Aria2 的下载目录一定要设置为用户可以访问的文件夹，否则会出现权限问题，导致无法下载

### 配置文件

```bash
# 建立文件夹以存放 aria2 的配置文件
mkdir ./.config/aria2/
# 创建配置文件
touch ./.config/aria2/aria2.conf
# 可以理解为当前下载的任务记录，避免丢失进度
touch ./.config/aria2/aria2.session
```

之后编辑配置文件: `vim ./.config/aria2/aria2.conf`, 可根据下文模板填写，也可以去查找文档
...
# Version: 2020.12.28
#
# Rwirited by: Sakura70s
# Rwirited Date: 2022.08.07
...
max-overall-download-limit=20480k

# 单任务下载速度限制, 默认：0 (无限制)
max-download-limit=18192k
...
# 设置过低可能影响 BT 下载速度
max-overall-upload-limit=4096k

# 单任务上传速度限制, 默认:0 (无限制)
max-upload-limit=2048k
...
# Aria2格式:https://cdn.jsdelivr.net/gh/DeSireFire/animeTrackerList/ATaria2_all.txt

# #Trackerlist
# https://github.com/ngosang/trackerslist
# https://github.com/XIU2/TrackersListCollection
# Aria2格式:https://cdn.jsdelivr.net/gh/XIU2/TrackersListCollection@master/all_aria2.txt
...
```bash
09/04 13:16:22 [NOTICE] IPv4 RPC：正在监听 TCP 端口 6800

09/04 13:16:22 [NOTICE] IPv6 RPC：正在监听 TCP 端口 6800
                                                                                                                                                                                                
09/04 13:16:23 [NOTICE] 已成功将当前会话序列化到“/xxxxxxx/.config/aria2/aria2.session”。
```
...
Group=root
Type=simple
# --conf-path= 后面换成自己写好的 Aria2 配置文件路径
